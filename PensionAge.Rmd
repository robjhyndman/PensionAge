...
aus.pop <- hmd.pop(country = "AUS", username = username, password = password, label = "Australia")
...
aus.mig <- netmigration(mort = aus.mort, fert = aus.fert, startyearpop = aus.pop, mfratio = 1.05)
...
pyramid <- function(history, future) {
  ...
  p_male <- data %>%
    ...
    ggplot(aes(x = Age, y = Population, group=Year)) +
      ...
      geom_line(linetype=2) + geom_line(aes(y=Mean), col='#0000cc') +
      coord_flip() +
...
for (i in seq(t - k - H))
{
    ...
    aus.sim.s <- pop.sim(mort = mortfs, fert = fertfs, mig = migfs,
...
pension_age_lower <- pension_age %>%
  filter(Policy == "Current", Year >= 2019) %>% select(-Policy)
for (i in seq(5,50)) {
...
pension_age_optimal %>%
  mutate(
    ...
    AgeLo = paste0(trunc(Lo80),"Y",
      if_else(MonthLo > 0, paste0("+", MonthLo, "M"), "")),
    AgeHi = paste0(trunc(Hi80),"Y",
