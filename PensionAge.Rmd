...
\begin{equation}
  \hat{y}_{n+h\mid n}(x)=\hat{s}_{n+h\mid n}(x) = \text{E}[\hat{s}_{n+h\mid n}(x)\mid \bm{I, B}] = \hat{\mu}(x)+\sum_{j=1}^{J}\phi_j(x)\hat{\beta}_{n+h\mid n,j}
\end{equation}
...
\begin{equation}
  \bm{p}^{work}_{n+1} = \left[\sum_{m=15}^{a-1}p_{m,1},\sum_{m=15}^{a-1}p_{m,2},\dots,\sum_{m=15}^{a-1}p_{m,1000}\right]
\end{equation}
Using the same method, the simulations of total number of aged pensioners in the population can be shown as:
\begin{equation}
  \bm{p}^{aged}_{n+1} = \left[\sum_{m=a}^{100^+}p_{m,1},\sum_{m=a}^{100^+}p_{m,2},\dots,\sum_{m=a}^{100^+}p_{m,1000}\right]
\end{equation}
...
aus.fert_v0 = extract.years(aus.fert, 1921:2014)

##### MIGRATION
aus.mig <- netmigration(mort = aus.mort, fert = aus.fert_v0, mfratio = 1.05)
```
...
ausfert.sm <- smooth.demogdata(aus.fert_v0, obs.var = "theoretical")
...
aus.sim <- pop.sim(mort = mortf, fert = fertf, mig = migf, firstyearpop = auspop, N = nsim)
```
...
for (i in 1:(t - k - H))
{
  ...
  aus.sim.s <- pop.sim(mort = mortfs, fert = fertfs, mig = migfs, firstyearpop = pop.short, 
                       N = 3)
