...
When we apply the @HU07 model to data on males and females, we need to constrain the resulting forecasts so they do not diverge. In the product-ratio method [@HBY13], we apply the HU model to the geometric mean of male and female log mortality data, and the ratio of male and female log mortality data, rather than to each series independently. That is, we define
\begin{align}
...
\end{align*}
where time series $\left\{\beta_{t,j}\right\}$ are fitted with an ARIMA model while $\left\{\gamma_{t,l}\right\}$ are fitted by stationary ARFIMA model (i.e., the order of differencing is constrained to $0<d<\frac{1}{2}$). It is the stationary constraint on $\gamma_{t,l}$ that ensures the forecasts of male and female do not diverge. Then, forecasts of sex-specific data are obtained using
\begin{align*}
...
We follow the general procedure proposed in @HB08, except that we use the product-ratio method rather than the HU method for mortality and net-migration.
...
## Forecasting OADR {#sec:forecasting}
