...
The Lee-Carter (LC) method has been widely used to forecast mortality rates across various countries, including Australia. The model proposed by @LC92 is
...
The model is over-parameterised. For example, if the vector $a,b,k$ is one solution, then for any scalar $c$, $a, b/c, kc$ or $a - bc, b, k+c$ also must be two possible solutions. Therefore,  the constraints
\begin{equation}
  \sum_{t=1}^{n}k_t=0, \qquad\text{and}\qquad \sum_{x=x_1}^{x_m}b_x=1
\end{equation}
...
When we apply the @HU07 model to data on males and females, we need to constrain the resulting forecasts so they do not diverge [@HBY13]. In the product-ratio method, we apply the HU model to the geometric mean of male and female log mortality data, and the ratio of male and female log mortality data, rather than to each series independently. That is, we define
\begin{align}
  p_t(x)&=\sqrt{s_{t}^{\text{M}}(x)s_{t}^{\text{F}}(x)}, \label{eq:coherent_1}\\
  r_t(x)&=\sqrt{\frac{s_{t}^{\text{M}}(x)}{s_{t}^{\text{F}}(x)}}. \label{eq:coherent_2}
\end{align}
Then, the HU method is applied to these two new variables instead of male and female series:
\begin{align*}
  \log[p_t(x)]=\mu_p(x)+\sum_{j=1}^{J}\beta_{t,j}\phi_j(x)+e_t(x)\\
  \log[r_t(x)]=\mu_r(x)+\sum_{l=1}^{L}\gamma_{t,l}\psi_l(x)+z_t(x)
\end{align*}
where time series $\left\{\beta_{t,j}\right\}$ are fitted with an ARIMA model while $\left\{\gamma_{t,l}\right\}$ are fitted by stationary ARFIMA model (i.e., $d$ in ARIMA($p,d,q$) constrained to $0<d<\frac{1}{2}$). It is the stationary constraint on $\gamma_{t,l}$ that ensures the forecasts of male and female do not diverge. Then, forecasts of sex-specific data are obtained using
\begin{align*}
...
The main procedure for forecasting the population age structure follows @HB08, except that the data are updated, and we incorporate the product-ratio method [@HBY13] to improve the accuracy of the forecast.

, introduced in section \@ref(sec:HU), is applied to the mortality rate, fertility rate and net migration separately, based on the assumption that they behave independently. Therefore, to fit the model to historical fertility rates, let $y_t(x)$ in equation \eqref{eq:HU_1} denote the observed historical fertility data.
