...
```{r totalpopulationin2034, fig.cap="Forecast population pyramid for 2034, along with 80% pointwise prediction intervals. The actual pyramid for 2015 is shown using dashed lines.", , dependson='simulationresults'}
par(mgp = c(2.2, 0.45, 0), tcl = -0.4, mar = c(4.3, 3.6, 3.0, 3.3))
plot(c(0, 100), c(0, 100),
  type = "n", main = "Forecast population: 2034", yaxt = "n",
...
oadr.real.mat <- oadr.error <- matrix(NA, 25, 15)
for (i in 1:25)
  oadr.real.mat[i, ] <- oadr.real[(i + 56):(56 + 14 + i)]

oadr.error <- abs(oadrtest - oadr.real.mat)
oadrave <- apply(oadr.error, 1, mean)

plot(oadrave, xlab = "forecast horizon", ylab = "mean", ylim = c(0, 0.02))
```
...
tab <- matrix("", nrow = 10, ncol = 15)
for (i in 1:5)
{
  tab[, (i - 1) * 3 + 1] <- as.character(year[(10 * (i - 1)) + (1:10)])
  tab[, (i - 1) * 3 + 2] <- age.lo[(10 * (i - 1)) + (1:10)]
  tab[, (i - 1) * 3 + 3] <- age.up[(10 * (i - 1)) + (1:10)]
}
colnames(tab) <- rep(c("Year", "Lo", "Hi"), 5)
kable_output = knitr::kable(format(tab),
  booktabs = TRUE,
...
kable_styling(kable_output, font_size = 7)
```
