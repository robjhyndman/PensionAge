...
pension_age %>%
  filter(year <= 2050) %>%
  ggplot(aes(x=year, y=age, group=policy)) +
    geom_line(aes(col=policy)) +
    xlab("Year") + ylab("Age of retirement") +
    scale_color_manual(
      values = c(current = "#1b9e77", approved = "#d95f02", proposed = "#7570b3"),
      name = " ",
...
auspop <- extract.ages(auspop, ages = 0:100)
...
aus.mig <- netmigration(mort = aus.mort, fert = aus.fert, startyearpop = auspop, mfratio = 1.05)
save(aus.fert, aus.mort, aus.mig, file="ausdata.Rdata")
```
...
auspop_df <- bind_rows(
  as_tibble(auspop$pop$male / 1000) %>%
    mutate(
      Age = auspop$age,
      Sex = "Male"
    ) %>%
    pivot_longer(c(-Age, -Sex), names_to = "Year", values_to = "Population"),
  as_tibble(auspop$pop$female / 1000) %>%
    mutate(
      Age = auspop$age,
      Sex = "Female"
    ) %>%
...
```{r simtibble, dependson = 'simulation}
...
history_tot_pop %>% 
  ggplot(aes(x=Year, y=Population, group=Sex)) +
    ...
    geom_line(data=future_tot_pop, col='blue', mapping=aes(y=Mean), size=1) 
```
...
pyramid <- function(history, future) {
  ...
  p_male <- data %>%
    ...
    ggplot(aes(x = Age, y = Population, group=Year)) +
      ...
      geom_line(linetype=2) + geom_line(aes(y=Mean), col='blue') + 
      coord_flip() +
...
current <- pension_age %>% 
  filter(policy=="current", year %in% unique(tot_pop$Year) ) %>% 
  arrange(year) %>% pull(age)
proposed <- pension_age %>% 
  filter(policy=="proposed", year %in% unique(tot_pop$Year) ) %>% 
  arrange(year) %>% pull(age)
approved <- pension_age %>% 
  filter(policy=="approved", year %in% unique(tot_pop$Year) ) %>% 
  arrange(year) %>% pull(age)
OADR.65 <- oadr(history, aussim_df, pension.age = current) %>% mutate(policy="Pension age 65")
OADR.65 %>%
  ggplot(aes(x=Year, y=OADP, group=Sex)) +
...
OADR.67 <- oadr(history, aussim_df, pension.age = approved) %>% 
  mutate(policy="Current policy")
OADR.prop <- oadr(history, aussim_df, pension.age = proposed) %>% 
  mutate(policy="Proposed policy")
bind_rows(OADR.65,OADR.67,OADR.prop) %>%
  mutate(policy = factor(policy, 
    levels = c("Pension age 65","Current policy","Proposed policy"))) %>%
  ggplot(aes(x=Year, y=OADP, group=policy)) +
    geom_ribbon(aes(ymin=Lo, ymax=Hi, fill=policy), alpha=0.5) +
    geom_line(aes(col=policy), size=1) +
